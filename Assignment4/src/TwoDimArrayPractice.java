...
for( int row1 = 0; row1 <intArray.length;row1++) {
	 ...
	 for(int col1 = 0; col1 <intArray[row1].length; col1++) {
		 if(row1 == row) {
		  
		intArray[row1][col1] = value;  
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	   int lowest = intArray[0][0];
	   for( int row = 0; row < intArray.length; row++) {
			 
			 for(int col = 0; col < intArray[row].length; col++) {
				 
				  if(col == column && intArray[row][col] < lowest) {
			
					lowest = intArray[row][col];
					
					
					 }
				  animate(row, col,column);
				 }
